...
def schedule():
    ...
    operating_days_filter = request.args.get('operations_days')
    stations = sorted(set([s['source'] for s in schedules] + [s['destination'] for s in schedules]))
...
def confirmation():
    ...
    operating_days = request.args.get('operating_days')
    time = request.args.get('time')
    return render_template('confirmation.html', name=name, bus_route_id=bus_route_id, source=source, destination=destination, operating_days=operating_days, time=time)
...
def cancel_booking(booking_id):
    ...
    flash(f'Your booking for {booking.bus_route_id} from {booking.source} to {booking.destination} on {booking.operating_days} at {booking.time} has been cancelled.', 'success')
    return redirect(url_for('my_bookings'))
...
def chatbot_api():
    ...
    try:
        ...
        - "schedule_query" (if they are asking for bus schedules, routes, times, or journeys between places, e.g., "buses from London", "schedule to Leeds", "bus")
        ...
        - "general_conversation" (for greetings, questions about current date, time, weather, or anything not directly about bus schedules or app functionality)

        Return only the category name. Do not include any other text.
        Query: "{user_message}"
        """

        intent_response = chat_session.send_message(intent_prompt)
        detected_intent = intent_response.text.strip().lower()

        final_ai_response = ""

        if detected_intent == "general_conversation":
            general_response = chat_session.send_message(user_message)
